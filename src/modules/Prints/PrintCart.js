            ...
            <table className="hesabfaMainTable">
              <tbody>
                ...
                <tr key={i}>
                  ...
                  <td>{normalPriceCount(items.total&&items.total.discount)}</td>
                  <td>{normalPriceCount(items.total&&items.total.tax)}</td>
                  <td>{normalPriceCount(items.total&&items.total.total)}</td>
                </tr>))}
              </tbody>
            </table>
              ...
              <div className="footerRows">
                  <div className="hesabfaPrice">
                    <div className="priceSeprate">
                      <span>مجموع:</span>
                      <span>{normalPriceCount(total&&total.totalFee) } ریال</span>
                    </div>
                    <div className="priceSeprate">
                      <span>تخفیف:</span>
                      <span>{normalPriceCount(total&&total.totalDiscount) } ریال</span>
                    </div>
                    <div className="priceSeprate">
                    <span>مالیات: </span>
                    <span>{normalPriceCount(total&&total.totalTax)+" ریال "}</span>
                    </div>
                    <hr/>
                    <h3>مبلغ کل: {normalPriceCount(total&&total.totalPrice)} ریال</h3>
                    <strong> </strong>
                  </div>
                </div>
            </div>
